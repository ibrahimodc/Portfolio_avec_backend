...
function Projet({ projet, onSupprimer, onAfficherDetail }) {
  ...
  const placeholder = `https://placehold.co/600x400/e0f2fe/0369a1?text=${encodeURIComponent(projet.libelle)}`;
  const imageSrc = imgError ? placeholder : (getImageUrl(projet.image) || placeholder);
  ...
  return (
    <article className="projet-carte">
      ...
      <div className="projet-corps">
        ...
        <Link to={`/projets/${projet._id}`} className="projet-libelle">
          {projet.libelle}
        </Link>
